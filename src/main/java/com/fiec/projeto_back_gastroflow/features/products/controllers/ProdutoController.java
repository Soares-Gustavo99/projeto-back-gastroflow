...
public class ProdutoController {
    ...
    @Operation(summary = "Insere uma imagem para o produto", description = "Permitido para 'ADMIN' e 'STANDARD'.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Imagem inserida com sucesso."),
            @ApiResponse(responseCode = "400", description = "Dados da imagem inválidos."),
            @ApiResponse(responseCode = "403", description = "Acesso negado.")
    })
    @ResponseStatus(HttpStatus.OK)
    @PutMapping("/{id}/photo") // Usa o ID do produto no path
    @PreAuthorize("hasAnyRole('ADMIN','STANDARD')")
    public void insertProdutoImage(@Parameter(description = "ID do Produto a ser deletado.") @PathVariable Long id,
